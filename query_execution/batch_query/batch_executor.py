...
Reads a CSV with:
  - 'query' or 'sql_text' column (SQL)
  - optional 'plan_json' column (execution plan)

Writes back a CSV with an 'execution_time' column (or 'execution_time_ms').
Used for RL fine-tuning reward computation (e.g. negative execution time).
"""
...
# Add single_query to path so we can import the executor
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(CURRENT_DIR)
SINGLE_QUERY_DIR = os.path.join(PROJECT_ROOT, "single_query")
...
def process_csv(csv_path, output_path, use_hints, iterations, workers, verbose):
    """Read CSV, run queries (optionally with hints), write execution times."""
    df = pd.read_csv(csv_path)

    # Support both 'query' and 'sql_text' column names (e.g. cardinal_dataset uses sql_text)
    query_col = "sql_text" if "sql_text" in df.columns else "query"
    if query_col not in df.columns:
        raise ValueError("CSV must have a 'query' or 'sql_text' column")
...
def main():
    ...
    parser.add_argument("csv", help="Path to CSV file containing queries (with 'query' or 'sql_text' column)")
    parser.add_argument("--output", "-o", help="Output CSV path")
    parser.add_argument("--use-hints", action="store_true", help="Use pg_hint_plan hints from plan_json")
